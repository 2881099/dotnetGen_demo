...
namespace cd.Module.Admin.Controllers {
	[Route("[controller]")]
	public class TesttypeparentinfoController : BaseController {
		public TesttypeparentinfoController(ILogger<TesttypeparentinfoController> logger) : base(logger) { }

		[HttpGet]
		async public Task<ActionResult> List([FromQuery] string key, [FromQuery] int limit = 20, [FromQuery] int page = 1) {
			var select = Testtypeparentinfo.Select
		...
		async public Task<ActionResult> Edit([FromQuery] int Id) {
			TesttypeparentinfoInfo item = await Testtypeparentinfo.GetItemAsync(Id);
		...
		async public Task<APIReturn> _Add([FromForm] string Name) {
			...
			item.Name = Name;
			item = await Testtypeparentinfo.InsertAsync(item);
		...
		async public Task<APIReturn> _Edit([FromQuery] int Id, [FromForm] string Name) {
			...
			if (item == null) return APIReturn.记录不存在_或者没有权限;
			item.Name = Name;
			int affrows = await Testtypeparentinfo.UpdateAsync(item);
		...
		async public Task<APIReturn> _Del([FromForm] int[] id) {
			...
			foreach (int id2 in id)
				affrows += await Testtypeparentinfo.DeleteAsync(id2);
