...
namespace cd.DAL {
	...
	public partial class Topiclazy : IDAL {
		#region transact-sql define
		public string Table { get { return TSQL.Table; } }
		public string Field { get { return TSQL.Field; } }
		public string Sort { get { return TSQL.Sort; } }
		internal class TSQL {
			internal static readonly string Table = "`topiclazy`";
			...
			internal static readonly string Delete = "DELETE FROM `topiclazy` WHERE ";
			...
			internal static readonly string InsertMultiFormat = @"INSERT INTO `topiclazy`(" + InsertField + ") VALUES{0}";
		...
		protected static MySqlParameter[] GetParameters(TopiclazyInfo item) {
		...
		public TopiclazyInfo GetItem(IDataReader dr) {
			int dataIndex = -1;
			return GetItem(dr, ref dataIndex) as TopiclazyInfo;
		}
		...
		private void CopyItemAllField(TopiclazyInfo item, TopiclazyInfo newitem) {
			item.Id = newitem.Id;
		...
		public SqlUpdateBuild Update(TopiclazyInfo item, string[] ignoreFields) {
			var sub = new SqlUpdateBuild(new List<TopiclazyInfo> { item });
		...
		public partial class SqlUpdateBuild {
			protected List<TopiclazyInfo> _dataSource;
			protected Dictionary<string, TopiclazyInfo> _itemsDic;
			protected string _fields;
			protected string _where;
			protected List<MySqlParameter> _parameters = new List<MySqlParameter>();
			public SqlUpdateBuild(List<TopiclazyInfo> dataSource) {
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 cd.DAL.Topiclazy.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public int ExecuteNonQuery() {
				...
				BLL.Topiclazy.RemoveCache(_dataSource);
				return affrows;
			}
			async public Task<int> ExecuteNonQueryAsync() {
				...
				await BLL.Topiclazy.RemoveCacheAsync(_dataSource);
				return affrows;
			}
			...
			public SqlUpdateBuild Set(string field, string value, params MySqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("cd.DAL.Topiclazy.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
		...
		public TopiclazyInfo Insert(TopiclazyInfo item) {
			if (int.TryParse(string.Concat(SqlHelper.ExecuteScalar(TSQL.Insert, GetParameters(item))), out var loc1)) item.Id = loc1;
			return item;
		}

		#region async
		async public Task<TopiclazyInfo> GetItemAsync(MySqlDataReader dr) {
			var read = await GetItemAsync(dr, -1);
			return read.result as TopiclazyInfo;
		}
		...
		async public Task<TopiclazyInfo> InsertAsync(TopiclazyInfo item) {
