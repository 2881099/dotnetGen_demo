...
namespace cd.DAL {
	...
	public partial class Tb_topic333 : IDAL {
		#region transact-sql define
		public string Table { get { return TSQL.Table; } }
		public string Field { get { return TSQL.Field; } }
		public string Sort { get { return TSQL.Sort; } }
		internal class TSQL {
			internal static readonly string Table = "`tb_topic333`";
			internal static readonly string Field = "a.`Id`, a.`Clicks`, a.`CreateTime`, a.`Title`";
			internal static readonly string Sort = "a.`Id`";
			internal static readonly string Returning = "; SELECT LAST_INSERT_ID();";
			internal static readonly string Delete = "DELETE FROM `tb_topic333` WHERE ";
			internal static readonly string InsertField = @"`Clicks`, `CreateTime`, `Title`";
			internal static readonly string InsertValues = @"?Clicks, ?CreateTime, ?Title";
			internal static readonly string InsertMultiFormat = @"INSERT INTO `tb_topic333`(" + InsertField + ") VALUES{0}";
		...
		protected static MySqlParameter[] GetParameters(Tb_topic333Info item) {
		...
		public Tb_topic333Info GetItem(IDataReader dr) {
			int dataIndex = -1;
			return GetItem(dr, ref dataIndex) as Tb_topic333Info;
		}
		...
		private void CopyItemAllField(Tb_topic333Info item, Tb_topic333Info newitem) {
			item.Id = newitem.Id;
		...
		public SqlUpdateBuild Update(Tb_topic333Info item, string[] ignoreFields) {
			var sub = new SqlUpdateBuild(new List<Tb_topic333Info> { item });
		...
		public partial class SqlUpdateBuild {
			protected List<Tb_topic333Info> _dataSource;
			protected Dictionary<string, Tb_topic333Info> _itemsDic;
			protected string _fields;
			protected string _where;
			protected List<MySqlParameter> _parameters = new List<MySqlParameter>();
			public SqlUpdateBuild(List<Tb_topic333Info> dataSource) {
			...
			public override string ToString() {
				...
				if (string.IsNullOrEmpty(_where)) throw new Exception("防止 cd.DAL.Tb_topic333.SqlUpdateBuild 误修改，请必须设置 where 条件。");
			...
			public int ExecuteNonQuery() {
				...
				BLL.Tb_topic333.RemoveCache(_dataSource);
				return affrows;
			}
			async public Task<int> ExecuteNonQueryAsync() {
				...
				await BLL.Tb_topic333.RemoveCacheAsync(_dataSource);
				return affrows;
			}
			...
			public SqlUpdateBuild Set(string field, string value, params MySqlParameter[] parms) {
				if (value.IndexOf('\'') != -1) throw new Exception("cd.DAL.Tb_topic333.SqlUpdateBuild 可能存在注入漏洞，不允许传递 ' 给参数 value，若使用正常字符串，请使用参数化传递。");
		...
		public Tb_topic333Info Insert(Tb_topic333Info item) {
			if (int.TryParse(string.Concat(SqlHelper.ExecuteScalar(TSQL.Insert, GetParameters(item))), out var loc1)) item.Id = loc1;
			return item;
		}

		#region async
		async public Task<Tb_topic333Info> GetItemAsync(MySqlDataReader dr) {
			var read = await GetItemAsync(dr, -1);
			return read.result as Tb_topic333Info;
		}
		...
		async public Task<Tb_topic333Info> InsertAsync(Tb_topic333Info item) {
